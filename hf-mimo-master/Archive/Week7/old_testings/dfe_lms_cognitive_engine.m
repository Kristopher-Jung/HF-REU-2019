...
function [params, e] = dfe_lms_cognitive_engine(Xest, dPsk, d, M)
% Initialize objective Function.
f = @(param)my_dfe_lms(Xest, dPsk, d, param(1), param(2),param(3), M);
% Options for GA CE.
...
opts = optimoptions(...
    @ga,...
    'PopInitRange',[lb;ub],'Generations',50,'StallGenLimit',20,...
    'PopulationSize',50,'CreationFcn','gacreationuniform',...
    'CrossoverFcn', 'crossoverscattered');
[params,e,~,~,~] = ga(f,n,[],[],[],[],lb,ub, [], intvars, opts);
end

function e = my_dfe_lms(Xest, dPsk, d, fwtaps, fbtaps,ss, M)
eq = comm.DecisionFeedbackEqualizer('Algorithm','LMS', ...
    'NumForwardTaps',fwtaps,'NumFeedbackTaps',fbtaps,'ReferenceTap',1, 'StepSize', ss,...
