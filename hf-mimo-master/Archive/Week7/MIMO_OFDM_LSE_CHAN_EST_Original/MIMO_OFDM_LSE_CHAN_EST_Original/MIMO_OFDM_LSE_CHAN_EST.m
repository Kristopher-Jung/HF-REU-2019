...
ofdm.Nb      = 1e2;                 % number of blocks
ofdm.Nt      = 4;                   % number of transmit antenna
...
if ofdm.DL > 0
    for nt = 1 : ofdm.Nt
        ofdm.dMod(ofdm.DPos,:,nt) = pskmod(ofdm.d(:,:,nt),ofdm.Mod, pi/ofdm.Mod);%%%%%%%%
    end
end
%% Pilot insertion
for nt = 1 : ofdm.Nt
...
wattersonChannel = stdchan('iturHFMQ',2000,1);%%%%%%%%%%%%%%%%%%%%%%
...
for nb = 1 : ofdm.Nb
    for nt=1:ofdm.Nt
        for nr=1:ofdm.Nr
            ofdm.Y(:,nb,nr) = ofdm.Y(:,nb,nr) + wattersonChannel(ofdm.ifftG(:,nb,nt));%%%%%%%%%%%
            ofdm.Y(:,nb,nr) = awgn(ofdm.Y(:,nb,nr),chan.SNR_dB,'measured');
        end
    end
end
...
if ofdm.ifDemodulateData == 1 && ofdm.DL > 0
    ...
    eq = comm.DecisionFeedbackEqualizer('Algorithm','LMS', ...
        'NumForwardTaps',4,'NumFeedbackTaps',4,'StepSize',0.01, ...
        'ReferenceTap',1);
    for nb = 1 : ofdm.Nb
        for dl = 1 : ofdm.DL
            ofdm.dDemod(dl,nb,:) = pinv(reshape(chan.CoeffEstFreq(ofdm.DPos(dl),:,:,nb),ofdm.Nt,ofdm.Nr).')...
                *squeeze(ofdm.fft(ofdm.DPos(dl),nb,:));
            %ofdm.dDemod(dl,nb,:) = eq(squeeze(ofdm.fft(ofdm.DPos(dl),nb,:)), squeeze(ofdm.dDemod(ofdm.DPos(dl),nb,:)));
        end
    end
    %scatterplot(ofdm.dDemod(:));
    % detection
    ofdm.dEst = zeros(ofdm.DL,ofdm.Nb,ofdm.Nt);
    for nt = 1 : ofdm.Nt
        ofdm.dEst(:,:,nt) = pskdemod(ofdm.dDemod(:,:,nt),ofdm.Mod, pi/ofdm.Mod);
    end
